...
func (this *appender) AppendByStream(s io.Reader, total int64) error {
	...
	fileinfo, err := newFileInfo("", nil, stream, this.fileExtName)
	if err != nil {
		return err
	}
	...
	if this.storageAddr == "" {
		err = this.appenderInit(fileinfo)
	} else {
		err = this.append(fileinfo)
	}
...
func (this *appender) AppendByFileName(fileName string) error {

	fileinfo, err := newFileInfo(fileName, nil, nil, this.fileExtName)
	if err != nil {
		return err
	}
	defer fileinfo.Close()
...
func (this *appender) appenderInit(f *fileInfo) error {
...
func (this *appender) append(f *fileInfo) error {
